...
Micro apps are rendered inside a super app. The super app controls its own theme (NativeWind global color scheme and navigation theme). Micro apps MUST NOT read or mutate the host’s global theme. Theme changes inside a micro app must be isolated to this micro app subtree.

### Rules (must follow)

- Do NOT call `toggleColorScheme()` or `setColorScheme()` from `nativewind` inside this micro app. These mutate the global scheme and will flip the host theme.
- Do NOT rely on `dark:` utility variants for dynamic theming in this micro app. `dark:` uses the global scheme; it will not follow this micro app’s local toggle.
...
- DO wrap this micro app root with `MicroThemeProvider` and keep it in place.
  ...
  - It exposes `useMicroTheme()` with `scheme`, `setScheme`, `toggle` for local toggles.
...
- Avoid hard-coded colors. Always use design tokens mapped in:
  - `providers/micro-theme` overriden colors
...
- Always make all colors in sync in all files.

### Local toggle usage

- Use `useMicroTheme()` to drive the toggle button inside this micro app:
